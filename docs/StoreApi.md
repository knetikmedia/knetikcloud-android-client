...
# **createStoreItem**
> StoreItem createStoreItem(storeItem)
...
try {
    StoreItem result = apiInstance.createStoreItem(storeItem);
    System.out.println(result);
} catch (ApiException e) {
...
# **updateItemTemplate**
> updateItemTemplate(id, itemTemplateResource)
...
try {
    apiInstance.updateItemTemplate(id, itemTemplateResource);
} catch (ApiException e) {
...
StoreItem storeItem = new StoreItem(); // StoreItem | The store item object
try {
    apiInstance.updateStoreItem(id, storeItem);
} catch (ApiException e) {
