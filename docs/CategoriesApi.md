...
try {
    PageResourceTemplateResource result = apiInstance.getArticleTemplatesUsingGET1(size, page, order);
    System.out.println(result);
} catch (ApiException e) {
...
String order = "id:ASC"; // String | A comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC]
try {
    PageResourceCategoryResource result = apiInstance.getCategoriesUsingGET1(filterSearch, filterActive, size, page, order);
    System.out.println(result);
} catch (ApiException e) {
...
Integer page = 1; // Integer | The number of the page returned, starting with 1
try {
    PageResourcestring result = apiInstance.getTagsUsingGET(size, page);
    System.out.println(result);
} catch (ApiException e) {
