...
try {
    PageResourceTemplateResource result = apiInstance.getPollTemplatesUsingGET(size, page, order);
    System.out.println(result);
} catch (ApiException e) {
...
String order = "id:ASC"; // String | A comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC]
try {
    PageResourcePollResource result = apiInstance.getPollsUsingGET(filterCategory, filterTagset, filterText, size, page, order);
    System.out.println(result);
} catch (ApiException e) {
