...
[**getQuestionTags1**](GamificationTriviaApi.md#getQuestionTags1) | **GET** /trivia/tags | List and search tags by the beginning of the string
...
<a name="getQuestionTags1"></a>
# **getQuestionTags1**
> Collectionstring getQuestionTags1(filterSearch, filterCategory, filterImportId)
...
try {
    Collectionstring result = apiInstance.getQuestionTags1(filterSearch, filterCategory, filterImportId);
    System.out.println(result);
} catch (ApiException e) {
    System.err.println("Exception when calling GamificationTriviaApi#getQuestionTags1");
    e.printStackTrace();
}
```
...
# **getQuestions**
> PageResourceQuestionResource getQuestions(size, page, order, filterSearch, filterIdset, filterCategory, filterTagset, filterType, filterPublished, filterImportId)
...
Long filterImportId = 789L; // Long | Filter for questions from a specific import job
try {
    PageResourceQuestionResource result = apiInstance.getQuestions(size, page, order, filterSearch, filterIdset, filterCategory, filterTagset, filterType, filterPublished, filterImportId);
    System.out.println(result);
} catch (ApiException e) {
...
> updateQuestionTemplate(id, questionTemplateResource)
...
QuestionTemplateResource questionTemplateResource = new QuestionTemplateResource(); // QuestionTemplateResource | The question template resource object
try {
    apiInstance.updateQuestionTemplate(id, questionTemplateResource);
} catch (ApiException e) {
