...
# **setClientRoles**
> setClientRoles(clientKey, rolesList)
...
try {
    apiInstance.setClientRoles(clientKey, rolesList);
} catch (ApiException e) {
...
> setPermissionsForRole(role, permissionsList)
...
try {
    apiInstance.setPermissionsForRole(role, permissionsList);
} catch (ApiException e) {
...
List<String> rolesList = Arrays.asList(new List<String>()); // List<String> | The list of unique roles
try {
    apiInstance.setUserRoles(userId, rolesList);
} catch (ApiException e) {
...
RoleResource roleResource = new RoleResource(); // RoleResource | The role resource object
try {
    apiInstance.updateRole(role, roleResource);
} catch (ApiException e) {
